document.addEventListener('DOMContentLoaded', function () {
    ...
    document.body.appendChild(SI.loader);
    setTimeout(function(){
        SI.loader.style.visibility = 'visible';
    }, 1000)
})
...
function SerpImport() {
    ...
    this.loader.style.left = 'absolute';
    this.loader.style.visibility = 'hidden';
    this.loader.style.left = '50vw';
    this.loader.style.marginLeft = '-2.5em';
    this.loader.style.width = '5em';
    this.loader.style.top = '5vh';
    this.loader.style.opacity = 0;
    this.loader.style.textTransform = 'uppercase';
    this.loader.style.letterSpacing = '0.1em';
    this.loader.style.fontFamily = 'Helvetica, Arial, sans';
    this.loader.style.fontSize = '20px';
    this.loader.style.color = '#333';
    this.loader.style.transition = 'all 0.2s';
    this.loaderDots = '';
}
...
SerpImport.prototype.getPaths = function(){
    ...
    var currentSrc = this.currentSrc = this.currentScript.src;
    this.root = {
...
SerpImport.prototype.definePlatform = function(){
    ...
    var platformReg = /\/[^\.]+\.([^\.]+)\.[^\.]+$/;
    if (platformReg.test(window.location.pathname)) {
        platform = window.location.pathname.match(platformReg)[1];
    }
    if (this.config.url['platform']) {
        platform = this.config.url['platform'];
    }
...
SerpImport.prototype.loadFiles = function(){
    ...
    var that = this;
    this.total = this._js.length + this._css.length;
    this.loaded = 0;

    window.addEventListener('lazyloaded', function(e){
        that.loaded++;
        // that.loader.style.top = (5 + that.loaded/that.total*90)+'vh';
        that.loader.style.opacity = (1 - that.loaded/that.total*0.8);
    })
...
SerpImport.prototype.onLibsLoaded = function(){
    if (this.isReady.js && this.isReady.css) {
        jBlock._onDomReady();
        try {
            document.body.removeChild(SI.loader);
        } catch (err) {

        }
        clearInterval(this.loaderAnimationTimer);
...
SerpImport.prototype.tryLoadFromYaml = function() {
    ...
    if (/noproject/.test(this.currentSrc)) return false;
